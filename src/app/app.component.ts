...
export class AppComponent {
  ...
  public handleFormSubmit(show: Show): void {
    if (this.showToEdit) {
  ...
  public createElement(show: Show): void {
    if (this.showToEdit) {
      const index = this.shows.findIndex(s => s.name === this.showToEdit?.name);
      if (index !== -1) {
    ...
    this.showToEdit = null; // Reset form after update or create
  }
  ...
  public updateElement(show: Show): void {
    ...
    this.showToEdit = null; // Reset form after update
  }
  ...
  public sortShowsAlphabetically(): void {
    this.shows = this.shows.sort((a, b) => a.name.localeCompare(b.name));
  }

  public reverseShowOrder(): void {
    this.shows = this.shows.reverse();
  }
}